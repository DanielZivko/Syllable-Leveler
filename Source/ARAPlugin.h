...
// Adapted from JUCE 8.0.6 ARAPluginDemo: syllable gain prototype.
#pragma once
                                                                ...
                                                                const std::function<AudioFormatReader* (ARAAudioSource*)>& getReader)
{
    ...
    const auto audioModification = playbackRegion->getAudioModification<SyllableAudioModification>();
    ...
    const Range<int64_t> sampleRangeInAudioModification {
        ...
        ARA::roundSamplePosition (rangeInAudioModificationTime.getEnd() * audioModificationSampleRate)
    };
    ...
    */
    ...
    {
        audioModification->apply (buffer, (int) outputOffset, (int) readLength, inputOffset);
...
public:
                       ...
                       const AudioPlayHead::PositionInfo& positionInfo) noexcept override
    {
        ...
        if (! lock.isLocked()) { buffer.clear(); return true; }
        ...
        if (isPlaying)
        {
            ...
            for (const auto& playbackRegion : getPlaybackRegions())
            {
                ...
                const bool mismatch = std::make_tuple (audioSource->getChannelCount(), audioSource->getSampleRate()) != std::make_tuple (numChannels, sampleRate);
                playbackRegion->getAudioModification<SyllableAudioModification>()->formatMismatch.store(mismatch);
                if (mismatch
                ...
                // Apply the per-segment gain map in original source coordinates
                playbackRegion->getAudioModification<SyllableAudioModification>()->apply (readBuffer, startInBuffer, numSamplesToRead, startInSource);
        ...
        (void) success;
        return true; // ARA owns the buffer, including silent unavailable regions.
    }
...
public:
    ...
    */
                       ...
                       const AudioPlayHead::PositionInfo& positionInfo) noexcept override
    {
        ...
        {
            if (! locked) { buffer.clear(); return true; }
            ...
            if (const auto previewedRegion = previewState->previewedRegion.load())
            {
                ...
                if (regionIsAssignedToEditor)
                {
                    ...
                    const auto previewDimmed = previewedRegion->getAudioModification<SyllableAudioModification>()
                                                              ->isEnabled();
...
//==============================================================================
class SyllableDocumentControllerSpecialisation final : public ARADocumentControllerSpecialisation,
...
protected:
    ...
    {
        return new SyllableAudioModification (audioSource,
                                                   hostRef,
                                                   static_cast<const SyllableAudioModification*> (optionalModificationToClone));
    }
    ...
    {
        try {
            if (input.readInt() != 1) return false;
            const auto count = input.readInt();
            if (count < 0 || count > 10000) return false;
            for (int i = 0; i < count; ++i) {
                const auto idLength = input.readInt();
                if (idLength <= 0 || idLength > 4096) return false;
                MemoryBlock bytes(static_cast<size_t>(idLength));
                if (input.read(bytes.getData(), idLength) != idLength) return false;
                const auto id = String::fromUTF8(static_cast<const char*>(bytes.getData()), idLength);
                syllable::Analysis a; bool enabled = false;
                if (!SyllableAudioModification::readArchive(input, a, enabled)) return false;
                if (auto* mod = filter->getAudioModificationToRestoreStateWithID<SyllableAudioModification>(id.getCharPointer()))
                    mod->restore(std::move(a), enabled);
            }
            return !input.failed();
        } catch (...) { return false; }
    }
    ...
    {
        try {
            const auto& mods = filter->getAudioModificationsToStore<SyllableAudioModification>();
            if (!output.writeInt(1) || !output.writeInt(static_cast<int>(mods.size()))) return false;
            for (const auto* mod : mods) {
                const String id(mod->getPersistentID());
                const auto length = id.getNumBytesAsUTF8();
                if (length == 0 || length > 4096 || !output.writeInt(static_cast<int>(length))
                    || !output.write(id.toRawUTF8(), length) || !mod->writeArchive(output)) return false;
            }
            return true;
        } catch (...) { return false; }
    }
...
//==============================================================================
class SyllableAudioProcessorImpl : public AudioProcessor,
                                        public AudioProcessorARAExtension
{
public:
    //==============================================================================
    SyllableAudioProcessorImpl()
        : AudioProcessor (getBusesProperties())
    {}

    ~SyllableAudioProcessorImpl() override = default;
    ...
    void processBlock (AudioBuffer<float>& buffer, MidiBuffer& midiMessages) override
    {
        ...
        playHeadState.update (audioPlayHead != nullptr ? audioPlayHead->getPosition() : nullopt);
    ...
    //==============================================================================
    const String getName() const override                             { return "Syllable Leveler"; }
...
private:
    ...
    JUCE_DECLARE_NON_COPYABLE_WITH_LEAK_DETECTOR (SyllableAudioProcessorImpl)
};
...
public:
        ...
        : araEditorView (editorView), playbackRegion (region), waveformCache (cache), previewRegionOverlay (*this)
    {
        ...
        setTooltip ("Duplo clique: analisar / comparar original. Botao direito: intensidade e reanalise. Use com transporte parado.");
    }
    ...
    {
        if (!m.mods.isPopupMenu()) return;
        PopupMenu menu;
        menu.addItem(1, "Analisar novamente");
        menu.addItem(2, "Ouvir original / processado");
        menu.addSeparator();
        menu.addItem(3, "Intensidade: 25%");
        menu.addItem(4, "Intensidade: 50%");
        menu.addItem(5, "Intensidade: 75%");
        menu.addItem(6, "Intensidade: 100%");
        const Component::SafePointer<PlaybackRegionView> safe(this);
        menu.showMenuAsync(PopupMenu::Options().withTargetComponent(this), [safe](int result) {
            if (safe == nullptr || result == 0) return;
            auto* mod = safe->playbackRegion.getAudioModification<SyllableAudioModification>();
            if (result == 1) mod->analyseSource();
            else if (result == 2) mod->setEnabled(!mod->isEnabled());
            else { mod->setAmount((result-2)*0.25); }
            mod->notifyEdit(); safe->repaint();
        });
    }
    ...
    {
        if (m.mods.isPopupMenu()) return;
        auto* mod = playbackRegion.getAudioModification<SyllableAudioModification>();
        if (!mod->hasAnalysis()) mod->analyseSource();
        else mod->setEnabled(!mod->isEnabled());
        mod->notifyEdit(); repaint();
    }
    ...
    void paint (Graphics& g) override
    {
        ...
        const auto* audioModification = playbackRegion.getAudioModification<SyllableAudioModification>();
        g.setColour (audioModification->isEnabled() ? Colours::darkgrey.darker() : Colours::darkgrey.brighter());
        ...
        const auto map = audioModification->snapshot();
        const double duration = playbackRegion.getDurationInAudioModificationTime();
        if (map.sampleRate > 0 && duration > 0) {
            const auto sourceStart = playbackRegion.getStartInAudioModificationTime();
            for (const auto& segment : map.segments) {
                const double t0 = static_cast<double>(segment.start)/map.sampleRate - sourceStart;
                const double t1 = static_cast<double>(segment.end)/map.sampleRate - sourceStart;
                if (t1 < 0 || t0 > duration) continue;
                const auto x0 = roundToInt(t0/duration*getWidth());
                const auto x1 = roundToInt(t1/duration*getWidth());
                g.setColour(segment.gainDb >= 0 ? Colours::turquoise : Colours::orange);
                g.drawVerticalLine(x0, 18.0f, static_cast<float>(getHeight()-18));
                if (x1-x0 > 42)
                    g.drawText(String(segment.gainDb, 1)+" dB", x0+2, 20, x1-x0-4, 16, Justification::centredLeft);
            }
        }
        g.setColour(Colours::white);
        const auto mode = audioModification->isEnabled() ? "PROCESSADO " : "ORIGINAL ";
        g.drawText(String(mode) + String(roundToInt(map.settings.amount*100)) + "% | " + audioModification->status,
                   getLocalBounds().removeFromBottom(18), Justification::centredLeft);
        if (audioModification->formatMismatch.load()) {
            g.setColour(Colours::red);
            g.drawText("Formato incompativel: fonte e sessao precisam da mesma taxa e numero de canais",
                       getLocalBounds().reduced(4), Justification::centred);
        }
...
private:
    ...
    SyllableDocumentControllerSpecialisation* getDocumentController() const
    {
        return ARADocumentControllerSpecialisation::getSpecialisedDocumentController<SyllableDocumentControllerSpecialisation> (playbackRegion.getDocumentController());
    }
...
constexpr auto trackHeight = 100;
...
class SyllableProcessorEditor final : public AudioProcessorEditor,
                                           public AudioProcessorEditorARAExtension
{
public:
    explicit SyllableProcessorEditor (SyllableAudioProcessorImpl& p)
    ...
    void paint (Graphics& g) override
    {
        ...
        if (! isARAEditorView())
        {
            ...
            g.drawFittedText ("ARA nao detectado. No Logic, use o primeiro slot de Audio FX. Em Apple Silicon, abra o Logic via Rosetta.",
                              getLocalBounds(),
...
private:
    ...
    JUCE_DECLARE_NON_COPYABLE_WITH_LEAK_DETECTOR (SyllableProcessorEditor)
};

class SyllableAudioProcessor final : public SyllableAudioProcessorImpl
{
public:
    bool hasEditor() const override               { return true; }
    AudioProcessorEditor* createEditor() override { return new SyllableProcessorEditor (*this); }
};
